...
def main():
    ...
    sbn.heatmap(corr_matrix, annot=True, xticklabels=names, yticklabels=names, ax=ax, cbar=True, mask=mask, annot_kws={"size":14})
    plt.yticks(fontsize=13)
    plt.xticks(fontsize=13)

    #if not os.path.isdir(path_to_data + './reports/figures/session_plots'):
    #    os.mkdir('./reports/figures/session_plots')
